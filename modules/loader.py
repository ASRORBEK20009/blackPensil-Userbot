...
async def loadmod(_, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if url.startswith(
            "https://raw.githubusercontent.com/BleckPensil-Userbot/custom_modules/main/"
        ):
            module_name = url.split("/")[-1].split(".")[0]
        elif "/" not in url and "." not in url:
            module_name = url.lower()
            url = f"https://raw.githubusercontent.com/BlackPensil-Userbot/custom_modules/main/{module_name}.py"
        else:
            modules_hashes = requests.get(
                "https://raw.githubusercontent.com/BlackPensil-Userbot/custom_modules/main/modules_hashes.txt"
            ).text
            resp = requests.get(url)
            ...
            if hashlib.sha256(resp.content).hexdigest() not in modules_hashes:
                return await message.edit(
                    "<b>Only <a href=https://github.com/BlackPensil-Userbot/custom_modules/tree/main/modules_hashes.txt>"
                    "verified</a> modules or from the official "
                    "<a href=https://github.com/BlackPensil-Userbot/custom_modules>"
...
async def unload_mods(_, message: Message):
    ...
    if module_name.startswith(
        "https://raw.githubusercontent.com/BlackPensil-Userbot/custom_modules/main/"
    ):
...
async def load_all_mods(_, message: Message):
    ...
    modules_list = requests.get(
        "https://api.github.com/repos/BlackPensil-Userbot/custom_modules/contents/"
    ).json()
...
async def updateallmods(_, message: Message):
    ...
    for module_name in modules_installed:
        ...
        resp = requests.get(
            f"https://raw.githubusercontent.com/BlackPensil-Userbot/custom_modules/main/{module_name}"
        )
        if not resp.ok:
