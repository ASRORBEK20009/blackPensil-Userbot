...
if [[ -d "BlackPensil-Userbot" ]]; then
  cd Dragon-Userbot
elif [[ -f ".env.dist" ]] && [[ -f "main.py" ]] && [[ -d "modules" ]]; then
  :
else
  git clone https://github.com/BlackPensil-Userbot/BlackPensil-Userbot || exit 2
  cd BlackPensil-Userbot || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  echo "It seems that BlackPensil-Userbot is already installed. Exiting..."
  exit
fi
...
echo
echo "============================"
echo "Great! BlackPensil-Userbot installed successfully!"
